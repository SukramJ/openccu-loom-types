...
openccu-loom-types. It is modeled on the
...
gates: the test suite via `make test` (runs `pytest -q`; CI's required `Test` check) and
`ruff check .` (ruff is a dev dependency; there is no `make lint` target). In addition, the
generated modules (`enums.py`, `rest.py`, `ws.py`, and the stamped block in `const.py`) must
never be hand-edited — CI's `Regen clean` check re-runs `make generate` and fails on any drift.
AI assistance does not lower the bar.

## How this project itself uses AI

In the interest of transparency: openccu-loom-types is partly developed with AI assistance
